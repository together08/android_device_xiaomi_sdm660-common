...
setup_vendor "$DEVICE_COMMON" "$VENDOR" "$EXTHM_ROOT" true
...
if [ -s "$MY_DIR"/../$DEVICE_SPECIFIED_COMMON/proprietary-files.txt ]; then
    ...
    setup_vendor "$DEVICE_SPECIFIED_COMMON" "$VENDOR" "$EXTHM_ROOT" true
...
if [ -s "$MY_DIR"/../$DEVICE/proprietary-files.txt ]; then
    ...
    INITIAL_COPYRIGHT_YEAR="$DEVICE_BRINGUP_YEAR"
    setup_vendor "$DEVICE" "$VENDOR" "$EXTHM_ROOT" false
